...
export default function AdminOrdersPage() {
  ...
  const filteredOrders = orders.filter(order => {
    ...
    const isPosOrder = order.metadata?.pos_sale === true ||
      order.order_items?.some((item: any) => item.metadata?.pos_sale === true);
    const matchesSource = sourceFilter === 'all' ||
      (sourceFilter === 'pos' && isPosOrder) ||
      (sourceFilter === 'website' && !isPosOrder);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Source filter — POS vs Website */}
      <div className="flex items-center gap-2">
        <span className="text-sm font-medium text-gray-500 mr-1">Source:</span>
        {(['all', 'website', 'pos'] as const).map((s) => (
          <button
            key={s}
          ...
          >
            {s === 'all' && <><i className="ri-list-check mr-1.5" />All</>}
            {s === 'website' && <><i className="ri-global-line mr-1.5" />Website</>}
            {s === 'pos' && <><i className="ri-store-2-line mr-1.5" />POS</>}
          </button>
        ))}
      </div>
                ...
                filteredOrders.map((order) => (
                  <tr key={order.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-4 px-4">
                      ...
                      {(order.metadata?.pos_sale === true || order.order_items?.some((item: any) => item.metadata?.pos_sale === true)) ? (
                        <span className="ml-2 inline-flex items-center px-1.5 py-0.5 rounded text-xs font-bold bg-purple-100 text-purple-700 border border-purple-200">
                          <i className="ri-store-2-line mr-1" />POS
                        </span>
                      ) : (
                        <span className="ml-2 inline-flex items-center px-1.5 py-0.5 rounded text-xs font-bold bg-stone-100 text-stone-600 border border-stone-200">
                          <i className="ri-global-line mr-1" />Web
                        </span>
                      )}
                    </td>
                    <td className="py-4 px-4">
