...
export default function ProductsPage() {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      // Stats always computed from active products only, using variant-aware stock
      const { data: allActive } = await supabase
        .from('products')
        .select('quantity, status, metadata, product_variants(quantity)')
        .eq('status', 'active');
      const { count: totalCount } = await supabase
        .from('products')
        .select('id', { count: 'exact', head: true });

      const activeList = (allActive || []).map((p: any) => {
        const variants: any[] = p.product_variants || [];
        const stock = variants.length > 0
          ? variants.reduce((sum: number, v: any) => sum + (v.quantity || 0), 0)
          : (p.quantity || 0);
        const threshold = p.metadata?.low_stock_threshold ?? 10;
        return { stock, threshold };
      });
      setStats({
        total: totalCount || 0,
        active: activeList.length,
        lowStock: activeList.filter(p => p.stock > 0 && p.stock <= p.threshold).length,
        outOfStock: activeList.filter(p => p.stock === 0).length,
      });
    } catch (error) {
